# Hybrid attempt
## When level 1, HGQOA for greed exploitation

## When level 0, Usual QOA behavior
...
__all__ = ["QuantityOriented_HalfGreedyHybrid"]


class QuantityOriented_HalfGreedyHybrid(OneShotAgent):
    ...
    def propose(self, negotiator_id: str, state):
        ...
        if self.awi.level == 1:
            if (
                ...
                return self.HG_best_offer(negotiator_id, step)
            ...
            if (
                ...
                return self.QOA_best_offer(negotiator_id, step)
            else:  # tries to settle using the best offer for its partners
                return self.quick_offer(negotiator_id, step)
    ...
    def respond(self, negotiator_id, state, offer, source=""):
        ...
        if my_needs <= 0:
            ...
            if self.awi.level == 1: # Half Greedy behavior
    ...
    def HG_best_offer(self, negotiator_id, step):
        my_needs = self._needed(negotiator_id)
        if my_needs <= 0:
            return None
        ami = self.get_nmi(negotiator_id)
    ...
    def QOA_best_offer(self, negotiator_id, step):
        my_needs = self._needed(negotiator_id)
        if my_needs <= 0:
            return None
        ami = self.get_nmi(negotiator_id)
