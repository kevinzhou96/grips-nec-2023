...
class AgentByQValue0717NonRandom1(OneShotAgent):
    ...
    def propose(self, negotiator_id: str, state):
        ...
        value_act = self.find_min_max_column(self.Q_propose_matrix, value_state)
        if True:
    ...
    def respond(self, negotiator_id, state, offer, source=""):
        ...
        value_act = self.find_max_reject_max_column(self.Q_respond_matrix, value_state)
        if True:
    ...
    def find_min_max_column(self, M, n):
        row_data = M[n]  # n行目の数値データを取得
        max_value = np.max(row_data)  # n行目の最大値を取得
        max_indices = np.where(row_data == max_value)[0]  # 最大値を持つ列のインデックスを取得
        min_column = np.min(max_indices)  # 列番号の最小値を取得
        return min_column

    def find_max_reject_max_column(self, M, n):
        row_data = M[n]  # n行目の数値データを取得
        max_value = np.max(row_data)  # n行目の最大値を取得
        max_indices = np.where(row_data == max_value)[0]  # 最大値を持つ列のインデックスを取得
        if 1 in max_indices:
            ...
            max_column = np.max(max_indices)  # 列番号の最小値を取得
            return max_column
